...
export class ListView
{
    ...
    private on_scrolling()
    {
        ...
        if(this.dir == ListViewDir.Vertical)
        {
            ...
            let viewport_start:number = -posy + this.height * this.mask.anchorY;
            let viewport_stop:number = viewport_start - this.height;
            while(this.items[start].y - this.item_height * this.item_tpl.anchorY - this.gap_y > viewport_start)
            {
                start++;
            }
            while(this.items[stop].y + this.item_height * this.item_tpl.anchorY < viewport_stop )
            {
                stop--;
            }
